"""
FPL MVP - ML pipeline + optimization

What this file does:
- Loads player data CSV (your format). Expects a CSV with columns you listed (e.g., now_cost, ep_next, form, position, team_name, player_name, etc.)
- Builds features for ML from available fields
- Trains a regression model (XGBoost if available, otherwise RandomForest) to predict next-GW points. By default it will use a target column named 'ep_next' if present; otherwise uses 'ep_next' as a proxy target (you should replace with real GW-labelled points when available).
- Predicts each player's expected points for the upcoming GW.
- Uses Integer Linear Programming (PuLP) to construct an optimal 15-player squad under FPL rules and budget, maximizing predicted points.
- Solves a second ILP to choose the best starting XI (dynamic formation) from that 15-man squad.
- Chooses captain & vice-captain.

How to use:
1. place your CSV file as `fpl_players.csv` (or change path)
2. pip install -r requirements.txt
   requirements.txt should include: pandas, numpy, scikit-learn, xgboost (optional), pulp
3. run: python fpl_mvp_ml.py

Notes & assumptions (MVP):
- now_cost in the FPL API is in tenths (e.g., 55 = 5.5). The code uses the same units and sets budget to 1000 = 100.0m.
- If you have a real 'ep_next' historical target column, the model will use it. If not, the code uses 'ep_next' (expected points) as a proxy target to let ML learn residuals from features.
- The optimization enforces: 15 players, 2 GKs, 5 DEF, 5 MID, 3 FWD, max 3 players per real-world team, budget <= 1000.
- Starting XI must respect formation rules (GK=1, DEF>=3, MID>=2, FWD>=1). The algorithm enumerates common formations and picks the best one.
...
# position name normalization (expecting the dataset to have 'position' like 'GK','DEF','MID','FWD')
VALID_POSITIONS = ['GKP', 'DEF', 'MID', 'FWD']

# Common valid formations to consider when choosing starting XI
FORMATIONS = [
...
def featurize(df):
    ...
    # Build features DataFrame at the very top
    ...
    # Normalize position values (handle GKP, etc.)
    ...
    # Only use specified predictive features for ML
    predictive_features = [
    ...
    # Normalize per-90 stats if raw and minutes available
    per90_map = {
    ...
    for raw, per90 in per90_map.items():
        if raw in features.columns and 'minutes' in features.columns:
            # If per90 not present, calculate
    ...
    # One-hot encode position for model
    for p in VALID_POSITIONS:
        features[f'pos_{p}'] = (features['position']==p).astype(int)

    # Keep reference to original df fields for output
...
def train_model(features, df, target_col='ep_next'):
    # If target exists in CSV, use it. Otherwise use ep_next as proxy target.
    ...
    # Clean missing values (already done in featurize)
    ...
    # Save predictions as 'predicted_points' for each player_id
...
def optimize_squad(df, features, preds, budget=BUDGET, max_per_team=MAX_PER_TEAM):
    # Build optimization problem to pick 15 players
    N = len(df)
    ...
    # Objective: maximize sum(predicted_points * pick)
    ...
    # Budget constraint (now_cost in tenths)
...
def pick_starting_xi_and_captain(squad):
    # From the 15-man squad, pick the best starting XI by enumerating formations
    best_lineup = None
    best_score = -1
    best_formation = None

    # Build helper lists by position
    ...
    for f in FORMATIONS:
        # skip formation if squad doesn't have enough players of a pos
        ok = True
        ...
        # select top predicted players per position counts
        lineup_idx = []
        # GKP - always 1
    ...
    # Order starting XI by position: GKP, DEF, MID, FWD
    ...
    # build bench: squad players not in starting XI, order by predicted points
...
def main():
    ...
    # Save predictions for each player_id
    pred_df.to_csv('predicted.csv', index=False)
    print("Saved predicted points to predicted.csv")
    ...
    # Only print starting XI and bench summary once
    ...
    # Print starting XI in order: GKP, DEF, MID, FWD
    pos_order = ['GKP', 'DEF', 'MID', 'FWD']
